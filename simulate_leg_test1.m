...
function qdot = discrete_impact_contact(z,p, rest_coeff, fric_coeff, yC)
    ...
    qdot = z(7:12);
    r_toe = position_toe(z,p); 
    v_toe = velocity_toe(z, p);
    r_heel= position_heel(z,p);
    v_heel = velocity_heel(z,p);
    
    if(r_heel(2)-yC<0 && v_heel(2) < 0)
      J  = jacobian_heel(z,p);
      A = A_leg(z,p);
      Ainv = inv(A);
      ...
      F_z = lambda_z*(-rest_coeff*v_heel(2) - J_z*qdot);
    ...
    z_test = z;
    z_test(7:12) = qdot;
    v_heel = velocity_toe(z_test, p);
    end

    if(r_toe(2)-yC<0 && v_toe(2) < 0)
      J  = jacobian_toe(z,p);
      A = A_leg(z,p);
      Ainv = inv(A);
      ...
      J_z = J(2,:);
      lambda_z = 1/(J_z * Ainv * J_z.');
      F_z = lambda_z*(-rest_coeff*v_toe(2) - J_z*qdot);
      qdot = qdot + Ainv*J_z.'*F_z;
