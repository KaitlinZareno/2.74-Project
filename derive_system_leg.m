...
syms x y t th1 th2 th3 ths dx dy dth1 dth2 dth3 dths ddx ddy ddth1 ddth2 ddth3 ddths real
...
p   = [m0 m1 m2 m3 m4 m5 ms I1 I2 I3 I4 I5 Is Ir N l_O_m1 l_B_m2 l_A_m3 l_C_m4 c5 l_OA l_OB l_AC l_DE l_IG l_GH l_heela ls l_C_s g k l_anklerest]';        % parameters
...
th2hat = cos(th2 + th1)*ihat - sin(th2 + th1)*jhat;
th3hat = cos(th3 + th2 + th1)*ihat - sin(th3+ th2 + th1)*jhat;
...
T3r = (1/2)*Ir*(dth1 + N*dth3)^2; %CHECK
Tsr = (1/2)*Ir*(N*dths)^2;

% ---------------------------------- %

%POTENTIAL ENERGY
P1 = m1*g*dot(rM1,-ihat);
P2 = m2*g*dot(rM2,-ihat);
P3 = m3*g*dot(rM3,-ihat);
P4 = m4*g*dot(rM4,-ihat);

%FOOT
P5 = m5*g*dot(rM5,-ihat);
...
Q = Q_tau1 + Q_tau2 + Q_tau3 + Q_taus + Q_tau2R + Q_tau3R;
