function simulate_system_leg()
    ...
    z0 = [-.03; 0.2; -pi/3; pi/2; 0; 0; 0; 0; 0; 0; 0; 0]; %moves with x velocity!
...
%          [z_out(7:12,i+1), condition(i)] = discrete_impact_contact(z_out(:,i+1),p, restitution_coeff, friction_coeff, ground_height);
        ...
        %z_out([4 10], i+1) = knee_constraint(z_out(:,i+1));
%         z_out([3 9], i+1) = hip_constraint(z_out(:,i+1));
...
%         z_out(1:2,i+1) = [0 0.2];
    ...
    hold on
    plot(tspan,x_offset -traj_r/2 * cos(w*tspan) ,'r--');
    plot(tspan,rE(2,:),'b','LineWidth',2)
    plot(tspan, y_offset + traj_r * sin(w*tspan) ,'b--');
    ...
    %desired circle
    TH = 0:.1:2*pi;
    xunit = x_offset + traj_r * cos(w*TH);
    yunit = y_offset + traj_r * sin(w*TH);
    plot(xunit, yunit);
...
function [F_toe, F_heel] = contact_friction(z,p, rest_coeff, fric_coeff, yC)
...
%     %stance leg, stagnant
    ...
    x_offset = -0.06;
    size = 0.04;
    rEd = [x_offset-size/2*cos(w*t+pi/2); y_offset + size*sin(w*t+pi/2)];
    r_opposite = atan(size/2*cos(w*t)/.13 - size*sin(w*t));
    rE = position_ankle(z,p);
    jE = jacobian_ankle(z,p);
    ...
    %circle trackin
        full_jacobian = -transpose(jE)*[K_x*(rE(1)-rEd(1))+D_x*vF(1);K_y*(rE(2)-rEd(2))+D_y*vF(2);0];
%         tau = [full_jacobian(3:4);0;Ks*(r_opposite-ts)+Ds*-ws];
        tau = [full_jacobian(3:4);0;Ks*(tds-ts)+Ds*-ws];
        tau(1) = tau(1)+pi/8;
    end
end
...
function Knee_res = knee_constraint(z)
    th2 = z(4);
    dth2 = z(10); 
    if th2 <= (15*pi/180)
        th2=15*pi/180;
...
% function hip_res = hip_constraint(z)
%     th1 = z(3);
%     dth1 = z(9); 
%     if abs(th1) > (pi)
%         th1=pi/3;
...
%     dth1 = z(7);  dth2= z(8);      dth3 = z(9);
    ...
    friction_coeff = 1;
    ground_height = 0;
    [friction_toe, friction_heel] = contact_friction(z,p, restitution_coeff, friction_coeff, ground_height);
   
    J_toe = jacobian_toe(z,p);
    J_heel = jacobian_heel(z,p);
        
    % Compute the contribution of the contact force to the generalied force
    QFc_t=  transpose(J_toe)*[friction_toe;Fc;0];
    QFc_h=  transpose(J_heel)*[friction_heel;Fh;0];
    QFc = QFc_t +  QFc_h; %toe and heel can both be in contact with ground
